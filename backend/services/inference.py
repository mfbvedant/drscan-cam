...
Pluggable model inference with demo mode (image-analysis) and custom model support.

Demo mode uses multi-feature image analysis to produce varying, plausible predictions
based on actual image characteristics. When a trained model is provided, swap via config.
"""
...
class InferenceEngine:
    """
    Pluggable inference engine supporting:
    - demo: image feature-based predictions (works immediately, no model needed)
    - custom: user-provided trained model (.h5 / .pt)
    """

    def __init__(self, mode: str = INFERENCE_MODE):
        self.mode = mode
        self.model = None
    ...
    # ------------------------------------------------------------------
    # Demo mode — image feature analysis
    # ------------------------------------------------------------------
    def _demo_predict(self, image: Image.Image, modality: str) -> Dict:
        ...
        Uses brightness, contrast, texture, and edge characteristics to
        weight probabilities across modality-specific classes.
        """
    ...
    def _extract_features(self, image: Image.Image) -> Dict[str, float]:
        """Extract meaningful visual features from the image."""
        ...
        # Color channel means (useful for fundus / pathology)
        r_mean = np.mean(rgb[:, :, 0]) / 255.0
        g_mean = np.mean(rgb[:, :, 1]) / 255.0
        b_mean = np.mean(rgb[:, :, 2]) / 255.0
        ...
        return {
            ...
            "contrast": contrast,
            "entropy": entropy / 6.0,  # Normalize (max ~6 for 64 bins)
            ...
            "g_mean": g_mean,
            "b_mean": b_mean,
            "center_bias": center_bias + 0.5,  # Shift to [0, 1]
        }
    ...
    def _feature_to_scores(
        ...
        Different feature combinations weight towards different pathologies.
        """
        n = len(classes)
        ...
        # Use a hash of features for consistent but varying results
        feature_hash = hash(
            (round(b, 2), round(c, 2), round(e, 2), round(ed, 3))
        ) % 1000 / 1000.0

        # Normal class: high brightness, moderate contrast, low asymmetry
        scores[0] = (b * 0.4 + (1 - a) * 0.3 + (1 - ed) * 0.3) * 2.5

        # Pathological classes: weighted by different feature combos
        for i in range(1, n):
            # Each pathology class responds to a different feature mix
        ...
        # Add small random-ish variation based on feature hash
        for i in range(n):
    ...
    def _confidence_to_severity(self, confidence: float, class_idx: int) -> int:
        ...
        # Higher confidence in pathology → higher severity
        if confidence > 0.85:
            return 4
    ...
    def _custom_predict(self, preprocessed: np.ndarray, modality: str) -> Dict:
        ...
        if self.model is None:
            raise RuntimeError("No custom model loaded. Call load_custom_model() first.")
